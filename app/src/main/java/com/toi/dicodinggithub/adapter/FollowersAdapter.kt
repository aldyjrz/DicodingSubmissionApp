...
class FollowersAdapter(
    ...
    class FollowersViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(userItems: Users) {
            with(itemView) {
                _rowUsernameFollowers.text = userItems.login
                Glide.with(context)
                    ...
                    .into(_rowAvatarFollowers!!)
                _rowUsernameFollowers.setOnClickListener{
                    Toast.makeText(context, _rowUsernameFollowers.text, Toast.LENGTH_SHORT).show()
                }
            }
